...
class ConfigManager
{
  ...
  public:
    ...
    bool Open(std::string indexFilepath, std::string dataFilepath)
    {
      ...
      this->dataIFStream.open(dataFilepath, std::fstream::binary);
    ...
    {
      this->indexOFStream.open(indexOutputFile, std::fstream::binary);
      this->dataOFStream.open(dataOutputFile, std::fstream::binary);

      if(!this->indexOFStream.good() || !this->dataOFStream.good())
        return false;

      this->opHandler.SetOutputStreams(this->indexOFStream, this->dataOFStream);
